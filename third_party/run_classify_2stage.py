...
from processors.marc import MarcProcessor
...
def train(args, train_dataset, model, tokenizer, lang2id=None):
  ...
  for name, param in model.named_parameters():
    for i in range(12):
      if '.'+str(i)+'.' in name or 'embeddings' in name:
  ...
  stage1_dataloader = DataLoader(train_dataset, shuffle=True, batch_size=args.train_batch_size*2)
  model.zero_grad()

  t_total_stage1 = len(stage1_dataloader) * args.num_stage1_epochs
  optimizer = AdamW(optimizer_grouped_parameters, lr=args.learning_rate, eps=args.adam_epsilon)
  scheduler = get_linear_schedule_with_warmup(
    optimizer, num_warmup_steps=args.warmup_steps, num_training_steps=t_total_stage1
  )

  train_iterator = trange(
    epochs_trained, int(args.num_stage1_epochs), desc="Epoch", disable=args.local_rank not in [-1, 0]
  )
  set_seed(args)  # Added here for reproductibility
  
  for _ in train_iterator:
    
    epoch_iterator = tqdm(stage1_dataloader, desc="Iteration", disable=args.local_rank not in [-1, 0])
    for step, batch in enumerate(epoch_iterator):
      ...
      outputs = model(**inputs,stage1=True)
      ...
      model.zero_grad()
  #torch.save(model, os.path.join(args.output_dir, "stage1.bin"))
  ...
  t_total = len(train_dataloader) // args.gradient_accumulation_steps * args.num_train_epochs
  optimizer = AdamW(model.parameters(), lr=5e-6, eps=args.adam_epsilon)
  ...
  for _ in train_iterator:
    ...
    for step, batch in enumerate(epoch_iterator):
      ...
      if args.model_type != "distilbert":
        ...
        #code_inputs["token_type_ids"] = (
        #  batch[2] if args.model_type in ["bert"] else None
        #)
      if args.model_type == "xlm":
        inputs["langs"] = batch[4]
      
      outputs = model(**inputs,dict=dict)
      ...
      if args.fp16:
        ...
        #    print(torch.any(torch.isnan(parms.grad)))
...
def main():
  ...
  parser.add_argument(
    "--num_stage1_epochs", default=3.0, type=float, help="Total number of training epochs to perform."
  )
  parser.add_argument(
    "--max_steps",
    default=-1,
